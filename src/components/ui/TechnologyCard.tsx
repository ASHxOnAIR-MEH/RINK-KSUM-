    ...
    <Link href={`/technologies/${technology.id}`} className="block group" id={`tech-card-${technology.id}`}>
      <div className="bg-card rounded-2xl border border-border overflow-hidden h-full flex flex-col hover:border-accent/30 hover:shadow-xl transition-all duration-300">
          ...
          <div className="relative h-36 overflow-hidden flex-shrink-0 bg-card-secondary">
            ...
            {isHigh && (
              <div className="absolute top-3 right-3">
                <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-[11px] font-black shadow-lg bg-[#E9C46A] text-[#112920]">
                  ★ FEATURED
                </span>
              </div>
            )}
        ...
        <div className="flex flex-col flex-1 p-5">
          {/* If no image, render sector and featured badge inline */}
          {!hasImage && (
            <div className="flex items-center justify-between mb-3">
              ...
              {isHigh && (
                <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-[11px] font-black shadow-sm bg-[#E9C46A] text-[#112920]">
                  ★ FEATURED
                </span>
              )}
            </div>
          ...
          {/* Dynamic Badges from Sheet (TRL, Patent, Commercialization) */}
          <div className="flex flex-wrap gap-1.5 mb-4">
            {/* TRL Level */}
            <span className="inline-flex items-center text-[10px] font-bold px-2 py-0.5 rounded bg-[#00FA9A]/10 text-[#00FA9A] border border-[#00FA9A]/20">
              {technology.trl.toLowerCase().includes('not available') 
                ? technology.trl 
                : (technology.trl.startsWith('TRL') ? technology.trl : `TRL ${technology.trl}`)}
            </span>
            
            {/* Patent Status */}
            <span className="inline-flex items-center text-[10px] font-bold px-2 py-0.5 rounded bg-[#E9C46A]/10 text-[#E9C46A] border border-[#E9C46A]/20">
