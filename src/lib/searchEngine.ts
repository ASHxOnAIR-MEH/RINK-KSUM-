 ...
 *   8. Sector match                            (score 300–320)
 *   9. Keyword match                           (score 160–220)
 *  10. Problem Solved match                    (score 110–130)
 *  11. Technology Type match                   (score 95)
 *  12. Orama full-text (Applications + Desc)   (score ≤80)
 *
...
async function getOramaDb(items: SearchIndexItem[]): Promise<AnyOrama> {
  ...
  for (const item of items) {
    await _insert!(db, {
      ...
      applications_str: item.applications.join(' '),
...
function scoreItem(item: SearchIndexItem, q: string): ScoredItem | null {
  ...
  const nSector      = norm(item.sector);
...
export async function precisionSearch(
  ...
  try {
    ...
    const oramaResult = await _search!(db, {
      term: q,
      properties: ['applications_str', 'description', 'keywords_str', 'problem_solved'],
      limit: 20,
